...
- **Source package:** `alertingframework.zip`

The supplied archive contains only a generic README and no alerting service, rules, integrations or AI implementation.
...
No model, detection logic, routing workflow, data contract or runtime evidence is available for verification.
...
Alerting intent → Missing source → Signal contract → Detection and routing design → Implementation → SLO validation
