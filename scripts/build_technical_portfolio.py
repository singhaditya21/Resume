...
def global_header(prefix: str, brand_context: str) -> str:
    links = (
        ("Approach", f"{prefix}index.html#approach"),
        ("Impact", f"{prefix}index.html#impact"),
        ("Work", f"{prefix}index.html#work"),
        ("Technical", f"{prefix}technical-projects.html"),
        ("Evidence", f"{prefix}build-evidence.html"),
        ("Cases", f"{prefix}case-study-library.html"),
        ("Slides", f"{prefix}presentation-archive.html"),
        ("Experience", f"{prefix}index.html#experience"),
        ("Contact", f"{prefix}index.html#contact"),
    )
    nav_links = []
    for label, href in links:
        current = ' aria-current="page"' if label == "Technical" else ""
        nav_links.append(f'<a href="{href}"{current}>{label}</a>')
    nav = "".join(nav_links)
    return f'''<header class="site-header" data-header><a class="brand" href="{prefix}index.html" aria-label="Aditya Singh, home"><span class="brand-mark">AS</span><span class="brand-copy"><strong>Aditya Singh</strong><small>{html.escape(brand_context)}</small></span></a><button class="menu-toggle" type="button" aria-expanded="false" aria-controls="site-nav"><span></span><span></span><span></span><span class="sr-only">Toggle navigation</span></button><nav id="site-nav" class="site-nav" aria-label="Primary navigation">{nav}</nav><a class="header-cta" href="{prefix}Aditya_Singh_Resume.pdf" target="_blank" rel="noopener">Resume <span aria-hidden="true">↗</span></a></header>'''
...
def validate_generated_site(public_out: Path, index_path: Path, concepts_path: Path, projects: list[dict]) -> None:
    ...
    for page in html_files:
        ...
        prefix = "" if page in {index_path, concepts_path} else "../../../"
        ...
        navigation = re.search(r'<nav id="site-nav" class="site-nav" aria-label="Primary navigation">(.*?)</nav>', page_text)
        if not navigation:
            raise RuntimeError(f"Missing global navigation contract in {page}")
        expected_links = [
            (f"{prefix}index.html#approach", "Approach"),
            (f"{prefix}index.html#impact", "Impact"),
            (f"{prefix}index.html#work", "Work"),
            (f"{prefix}technical-projects.html", "Technical"),
            (f"{prefix}build-evidence.html", "Evidence"),
            (f"{prefix}case-study-library.html", "Cases"),
            (f"{prefix}presentation-archive.html", "Slides"),
            (f"{prefix}index.html#experience", "Experience"),
            (f"{prefix}index.html#contact", "Contact"),
        ]
        actual_links = re.findall(r'<a href="([^"]+)"(?: aria-current="page")?>([^<]+)</a>', navigation.group(1))
        if actual_links != expected_links:
            raise RuntimeError(f"Global navigation destinations or labels differ in {page}")
        required_navigation_markers = (
            '<button class="menu-toggle" type="button" aria-expanded="false" aria-controls="site-nav">',
            f'<a class="brand" href="{prefix}index.html" aria-label="Aditya Singh, home">',
            f'<a href="{prefix}technical-projects.html" aria-current="page">Technical</a>',
            f'<a class="header-cta" href="{prefix}Aditya_Singh_Resume.pdf" target="_blank" rel="noopener">Resume ',
            f'<script src="{prefix}script.js"></script>',
        )
        for marker in required_navigation_markers:
            if page_text.count(marker) != 1:
                raise RuntimeError(f"Missing or duplicate global navigation marker {marker} in {page}")
